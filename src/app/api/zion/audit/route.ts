...
import { submitAudit, ZionError } from "@/lib/server/zion";
...
export async function POST(request: NextRequest) {
  ...
  // Optional pass-through fields. Drop anything unrecognised.
  const suiExecutionMode = form.get("suiExecutionMode");
  const notifyEmail = form.get("notifyEmail");
  ...
  if (typeof notifyEmail === "string" && notifyEmail)
    outbound.append("notifyEmail", notifyEmail);
  if (typeof webhookUrl === "string" && webhookUrl)
    outbound.append("webhookUrl", webhookUrl);

  try {
    const job = await submitAudit(outbound);
    await insertAudit({
      userId: user.id,
      job,
      sourceType: "upload",
      sourceMeta: {
        filenames,
        totalBytes,
        suiExecutionMode: typeof suiExecutionMode === "string" ? suiExecutionMode : undefined,
      },
    });
    ...
    logger.error("[zion/audit] unexpected error", err);
