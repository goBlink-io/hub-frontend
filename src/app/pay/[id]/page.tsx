    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
        <div className="card p-8 text-center max-w-sm">
          <AlertTriangle className="h-12 w-12 mx-auto mb-4" style={{ color: 'var(--warning)' }} />
      ...
      <div className="card p-6 sm:p-8 max-w-sm w-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            {paymentRequest.name && (
              <p className="text-body-sm" style={{ color: 'var(--text-muted)' }}>from {paymentRequest.name}</p>
            )}
          </div>
        </div>

        {/* Amount */}
        <div className="p-5 rounded-xl mb-5 text-center" style={{ background: 'var(--elevated)', border: '1px solid var(--border)' }}>
          <div className="text-tiny font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Amount requested</div>
        ...
        {paymentRequest.memo && (
          <div className="p-3 rounded-xl mb-5" style={{ background: 'var(--elevated)', border: '1px solid var(--border)' }}>
            <div className="text-tiny font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Memo</div>
            <p className="text-body-sm" style={{ color: 'var(--text-primary)' }}>{paymentRequest.memo}</p>
          </div>
        )}

        {/* Recipient */}
        <div className="p-3 rounded-xl mb-5" style={{ background: 'var(--elevated)', border: '1px solid var(--border)' }}>
          <div className="text-tiny font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Paying to</div>
          <code className="text-tiny font-mono break-all" style={{ color: 'var(--text-secondary)' }}>
        ...
        <div className="mt-6 pt-4 text-center" style={{ borderTop: '1px solid var(--border)' }}>
          <p className="text-tiny" style={{ color: 'var(--text-faint)' }}>
            Powered by <a href="https://goblink.io" target="_blank" rel="noopener noreferrer"
              className="font-semibold" style={{ color: 'var(--brand)' }}>goBlink</a>
          </p>
        </div>
      </div>
    </div>
